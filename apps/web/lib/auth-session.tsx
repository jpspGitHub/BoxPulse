...
import { createWebSupabaseClient } from "./supabase";
...
function toWebAuthUser(user: User): WebAuthUser {
  return {
    email: user.email ?? "",
...
function getSessionState(session: Session | null) {
  return {
    ...
    user: session ? toWebAuthUser(session.user) : null
  };
}
...
export function AuthSessionProvider({ children }: AuthSessionProviderProps) {
  ...
  const supabaseRef = useRef<ReturnType<typeof createWebSupabaseClient> | null>(null);
  ...
  const signOut = useCallback(async () => {
    const supabase = supabaseRef.current ?? createWebSupabaseClient();
...
export function useWebAuthSession() {
  ...
  if (!context) {
    throw new Error("useWebAuthSession must be used within AuthSessionProvider");
  }
